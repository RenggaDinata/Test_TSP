           ...
           <!-- /. NAV TOP  -->
            ...
            <div class="sidebar-collapse">
				...
				<li class="text-center">
                    ...
                    <li>
                        <a class="active-menu"  href="user-list.php"><i class="fa fa-table fa-3x"></i>List Data</a>
                    </li>
					<li>
										...
										<?php
											include 'koneksi.php';
												...
												if(isset($_GET['search'])){
													$search=$_GET['search'];
													$a=mysql_query("Select sheet_em.id, DATE_FORMAT(sheet_em.date1, '%d-%m-%Y') as tanggal1, DATE_FORMAT(sheet_em.date2, '%d-%m-%Y') as tanggal2, sheet_em.title, sheet_em.status, user_em.username, sheet_em.verify from sheet_em, user_em where sheet_em.pic=user_em.id and user_em.username='$login' and sheet_em.title like '%$search%' or sheet_em.keterangan1 like '%$search%' or sheet_em.keterangan2 like '%$search%' or sheet_em.status like '%$search%' or sheet_em.effects like '%$search%' or sheet_em.location like '%$search%' order by id");
												}else{
													$a=mysql_query("Select sheet_em.id, DATE_FORMAT(sheet_em.date1, '%d-%m-%Y') as tanggal1, DATE_FORMAT(sheet_em.date2, '%d-%m-%Y') as tanggal2, sheet_em.title, sheet_em.status, user_em.username, sheet_em.verify from sheet_em, user_em where sheet_em.pic=user_em.id and user_em.username='$login' order by id");
												}
												$no=1;
										...
										?>
												<tr>
													...
													<?php
													 ...
													 if($status == "closed")
													 {
														 echo "<a class='btn btn-success' href='user-view.php?id=$b[id]'> Finished </a>";
													 }
													 else
													 {
														if($hitung == 0 )
														{
															echo"<a class='button' href='user-edit.php?id=$b[id]'>Today !</a>";
												
														}
														elseif($hitung <= 0 )
															{
															echo"<a class='button' href='user-edit.php?id=$b[id]'>Late ". $hitung ." days </a>";
														
															//echo"<a class='button' href='user-edit.php?id=$b[id]'>Today !</a>";
															}
														else
														{
															echo "<a class='button2' href='user-edit.php?id=$b[id]'>". $hitung ." days again </a> ";
														}
													 }
													...
													<td align="center">
														<a href="user-edit.php?id=<?php echo $b['id'] ?>"><button class="btn btn-primary"><i class="fa fa-edit "></i> Ubah</button></a>
														<a href="hapus.php?id=<?php echo $b['id'] ?>" onclick="return confirm('Are you sure to DELETE this record ?')"><button class="btn btn-danger"><i class="fa fa-pencil"></i> Hapus</button></a>
